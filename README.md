# Salut, moi c'est Florian 👋

🎓 **En reconversion vers le développement informatique** après un parcours dans l'ingénierie du son, je suis actuellement à la recherche d'une **alternance pour préparer un Bachelor Concepteur Développeur d'Applications** dans la région de **Lille**.

Passionné par la technique et la création, la rigueur et la logique acquises dans l'audio me porteront vers le monde du code.

---

### 🎯 Mes objectifs

- 🏫 **Formation visée :** Bachelor Concepteur Développeur d'Applications (CDA)
- 📍 **Localisation :** Métropole Lilloise
- 💼 **Rythme recherché :** Alternance école / entreprise

---

#### En apprentissage / Découverte

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

---

### 🎵 Mon Parcours

- 🎛️ **Bachelor Technicien du Son** (3 ans dont 1 an en alternance) : Maîtrise du travail en équipe, rigueur technique, gestion de projets audio, fiches techniques et écoute client.
- 💻 **Reconversion Développeur** : Auto-formation proactive (FreeCodeCamp), création de projets personnels (GitHub) et préparation aux études supérieures.

---

### 🚀 Projets en cours

- ⚙️ Développement de petites applications en JavaScript & Python.
- 🤝 Volonté d'intervenir et de contribuer sur des projets open-source.

---

### 📬 Me contacter

- 💼 **LinkedIn :** [Florian Goudenhooft](https://www.linkedin.com/in/ableton/)
- ✉️ **Email :** [flogoudenhooft@gmail.com](mailto:flogoudenhooft@gmail.com)
